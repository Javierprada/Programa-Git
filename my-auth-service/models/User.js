...
class User {
    static async create(username, password, email) {
        try {
            ...
            const password_hash = await bcrypt.hash(password, 10); // El 10 es el costo del salting (rondas de hashing)
    ...
    static async updatePassword (userId, newPassword) {
        try {
            ...
            const [resul] = await pool.execute (
                'UPDATE users password_hash = ?, reset_password_token = NULL, reset_password_expire = NULL id = ?',
